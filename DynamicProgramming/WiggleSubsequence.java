...
public class WiggleSubsequence {
    public int wiggleMaxLength(int[] nums) {
        ...
        int same = 0;
        while (same < nums.length - 1 && nums[same] == nums[same + 1]) same++;
        if (nums.length - 1 == same) return 1;
        boolean increasing = nums[same + 1] > nums[same];
        int res = 1;
        int prev = nums[same];
        for (int i = same + 1; i < nums.length; i++) {
