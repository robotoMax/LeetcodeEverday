/**
 * There is a garden with N slots. In each slot, there is a flower. The N flowers will bloom one by one in 
 * N days. In each day, there will be exactly one flower blooming and it will be in the status of blooming 
 * since then.
...
public class KEmptySlots {
    public int kEmptySlots(int[] flowers, int k) {
        ...
        for (int i = 1; right < days.length; i++) {
            if (days[i] > days[right] && days[i] > days[left]) continue;
            if (i == right) {
